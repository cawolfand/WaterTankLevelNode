...
#define LED           BUILTIN_LED   // GPIO00/D3, onboard blinky for Adafruit Huzzah
#define USEACK true
...
//File: NodeWaterTankLevelWemoRFM915MHZRev2_MQTT.ino. : Based on combination of WaterTankLevel and 
//GatewayWemo915MHZRev2_MQTT.ino
//Uses WemoD1 controlling adafruit RFM69HCW to mostly transmit as a Node, but still has receive
//functionality (not tested)

#define trigPin 5    //Wemo pin D1
#define echoPin 16   //Wemo pinD0
...
//long distance;
int TRANSMITPERIOD = 4000; //transmit a packet to gateway so often (in ms)
...
void setup() {
  ...
  Serial.println();
  Serial.println("Starting NodeWaterTankLevelWemoRfm915MHZRev3.ino");
  delay(10);
  //EEPROM.begin(512);
...
delay(100);
  ...
  if (!radio.initialize(FREQUENCY,NODEID,NETWORKID)) {
    ...
    Serial.println("radio.initialize successfull");
  }
...
void loop() {
 ...
 int currPeriod = millis()/TRANSMITPERIOD;
  if (currPeriod != lastPeriod)
  {
       ...
       theData.sensorData1 = distance;
       theData.uptime = millis();
       Serial.print("Sending ");Serial.println(sizeof(theData));
       Serial.print("Device: "); Serial.print(theData.nodeId);
       Serial.print(" - Distance: "); Serial.print(theData.sensorData1);
       Serial.print(" ping_inch= "); Serial.println(distance);
  
  if (USEACK)
     {
      loops = 10;
      while ( loops--){
      if (radio.sendWithRetry(GATEWAYID,(const void*) (&theData), sizeof(theData))) { //target node Id, message as string or byte
   //array, message length
        Serial.println("   OK!!Good Send/rcv");
        Blink(50, 3); //blink LED 3 times, 50ms between blinks
        break;
      }
      else {
    ...
    {
       radio.send(NETWORKID,(const void*) (&theData), sizeof(theData));
    }
